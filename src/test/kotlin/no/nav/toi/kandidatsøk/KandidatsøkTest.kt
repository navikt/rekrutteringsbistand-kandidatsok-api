...
class KandidatsøkTest {
    ...
    @ParameterizedTest
    @MethodSource("endepunktSomParameter")
    fun `Kan søke kandidat med omfangkode deltid`(endepunkt: Endepunkt, wmRuntimeInfo: WireMockRuntimeInfo) {
        val wireMock = wmRuntimeInfo.wireMock
        mockES(wireMock, extraTerms = endepunkt.extraTerms + KandidatsøkRespons.omfangKodeBareDeltidTerm)
        endepunkt.ekstraMocking(wireMock)
        val navIdent = "A123456"
        val token = lagToken(navIdent = navIdent)
        val (_, response, result) = Fuel.post("http://localhost:8080/api/kandidatsok/${endepunkt.path}")
    ...
    @ParameterizedTest
    @MethodSource("endepunktSomParameter")
    fun `Kan søke kandidat med omfangkode heltid`(endepunkt: Endepunkt, wmRuntimeInfo: WireMockRuntimeInfo) {
        val wireMock = wmRuntimeInfo.wireMock
        mockES(wireMock, extraTerms = endepunkt.extraTerms + KandidatsøkRespons.omfangKodeBareHeltidTerm)
        endepunkt.ekstraMocking(wireMock)
        val navIdent = "A123456"
        val token = lagToken(navIdent = navIdent)
        val (_, response, result) = Fuel.post("http://localhost:8080/api/kandidatsok/${endepunkt.path}")
    ...
    @ParameterizedTest
    @MethodSource("endepunktSomParameter")
    fun `Kan søke kandidat med omfangkode både heltid og deltid`(endepunkt: Endepunkt, wmRuntimeInfo: WireMockRuntimeInfo) {
        val wireMock = wmRuntimeInfo.wireMock
